...
Firmware Overview
Calibration — sweep the robot over the line at startup (or via menu) to record min/max sensor readings per channel for normalization.
Sensor read loop — cycle S0–S3 through all 16 states, `analogRead()` on SIG after each, normalize, and compute a weighted line-position error.
PID loop — compute correction from position error using tunable Kp/Ki/Kd, apply as a differential PWM offset between PWMA and PWMB.
Menu system — OLED + 6 buttons for navigating start/stop, mode selection, and live PID constant adjustment.
Hardware Images
Full Schematic
...
Getting Started
Flash the firmware via USB (Arduino IDE or PlatformIO).
Power on via the slide switch.
Use the OLED menu to calibrate the sensor array on the actual track surface.
Tune PID constants via the menu, or hardcode starting values in firmware.
Select "Start" to begin autonomous line following.
